...
class TestJsonStore:
    ...
    def test_write_and_read(self, temp_store, sample_internal_audit_chunk):
        """Test writing and reading a chunk."""
        temp_store.write(sample_internal_audit_chunk)

        result = temp_store.read(sample_internal_audit_chunk.chunk_id)
        assert result is not None
        assert result["chunk_id"] == sample_internal_audit_chunk.chunk_id
        assert result["document_type"] == sample_internal_audit_chunk.document_type

    def test_write_batch(self, temp_store, sample_internal_audit_chunk):
        """Test writing multiple chunks."""
        chunks = [
            sample_internal_audit_chunk,
            Chunk(
...
class TestSqliteIndex:
    ...
    def test_insert_and_query_by_source(self, temp_db, sample_internal_audit_chunk):
        """Test inserting and querying by source type."""
        temp_db.insert(sample_internal_audit_chunk)

        results = temp_db.query_by_source("C")
        assert sample_internal_audit_chunk.chunk_id in results

    def test_insert_batch(self, temp_db, sample_internal_audit_chunk):
    ...
    def test_query_by_document(self, temp_db, sample_internal_audit_chunk):
        """Test querying by document ID."""
        temp_db.insert(sample_internal_audit_chunk)

        results = temp_db.query_by_document(sample_internal_audit_chunk.document_id)
        assert sample_internal_audit_chunk.chunk_id in results
    ...
    def test_insert_and_query_by_ticker(self, temp_db, sample_internal_audit_chunk):
        ...
        temp_db.insert_batch([sample_internal_audit_chunk, aapl_chunk, msft_chunk])
    ...
    def test_insert_and_query_by_standard_id(self, temp_db, sample_internal_audit_chunk):
        ...
        temp_db.insert_batch([sample_internal_audit_chunk, as1105_chunk, as2110_chunk])
    ...
    def test_query_by_ticker_no_match(self, temp_db, sample_internal_audit_chunk):
        """query_by_ticker for an unknown ticker returns an empty list."""
        temp_db.insert(sample_internal_audit_chunk)
        results = temp_db.query_by_ticker("XYZ")
        assert results == []

    def test_query_by_standard_id_no_match(self, temp_db, sample_internal_audit_chunk):
